...
const FolderItem = React.memo(({ item, level }: FolderItemProps) => {
    ...
    return (
        <div className="w-full">
            ...
            >
                ...
                {isFolder ? (
                    <Folder size={16} className={cn("shrink-0", item.isOpen ? "text-blue-400" : "text-zinc-500")} />
                ) : (
            ...
            {isFolder && item.isOpen && item.children && (
                <div className="flex flex-col">
                    {item.children.map((child) => (
                        <FolderItem key={child.id} item={child} level={level + 1} />
                    ))}
                </div>
            )}
        </div>
